...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 581431 - João Vitor de Souza", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // CABEÇA DO BONECO
        PushMatrix(model);
            model *= Matrix_Translate(0.0f, 0.05f, 0.f);
            PushMatrix(model);
                model = model // Atualizamos matriz model (multiplicação à direita) com a rotação da cabeça
                      ...
                      * Matrix_Rotate_X(-g_AngleX); // PRIMEIRO rotação X de Eule
                PushMatrix(model);
                    model *= Matrix_Scale(-0.3f, -0.3f, 0.4f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model);
            ...
            // Perna Esquerda
            PushMatrix(model);
                model *= Matrix_Translate(-0.17f, 0.0f, 0.0f);
                PushMatrix(model);
                    model *= Matrix_Scale(0.3f, 0.7f, 0.25f);
                    glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                    DrawCube(render_as_black_uniform); //quadriceps
                PopMatrix(model);
                PushMatrix(model);
                    model *= Matrix_Translate(0.0f, -0.75f, 0.0f);
                    PushMatrix(model);
                        model *= Matrix_Scale(0.25f, 0.7f, 0.3f);
                        glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                        DrawCube(render_as_black_uniform); //panturrilha
                    PopMatrix(model);
                    
                    PushMatrix(model);
                        model*=Matrix_Translate(0.0f, -0.75f, 0.0f);
                        PushMatrix(model);
                            model *= Matrix_Translate(0.0f, 0.0f, 0.1f);
                            PushMatrix(model);
                                model*=Matrix_Scale(0.2f, 0.1f, 0.4f);
                                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        PushMatrix(model); // Guardamos matriz model atual na pilha
            ...
            PushMatrix(model); // Guardamos matriz model atual na pilha
                ...
                PushMatrix(model); // Guardamos matriz model atual na pilha
                    ...
                    PopMatrix(model); // Tiramos da pilha a matriz model guardada anteriormente]
                    PushMatrix(model);
                        model = model * Matrix_Translate(0.0f,  -0.65, 0.0f);
                        PushMatrix(model);
